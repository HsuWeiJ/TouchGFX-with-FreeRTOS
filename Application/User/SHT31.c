...
//{
//	__HAL_TIM_SET_COUNTER(SHT31_TIMER, 0);
//	while ((__HAL_TIM_GET_COUNTER(SHT31_TIMER))<time);
//}
...
//		return false;
//	}
	if (HAL_I2C_Master_Transmit_DMA(handle->i2c_handle, handle->device_address << 1u, command_buffer, sizeof(command_buffer)) != HAL_OK)
	{
		return false;
	}
//	while (HAL_I2C_GetState(handle->i2c_handle) != HAL_I2C_STATE_READY);
