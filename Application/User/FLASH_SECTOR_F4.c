 ...
 *  Sector 12 to Sector 15 each 16KB
 *  Sector 16 as 64KB
 *  Sector 17 to Sector 23 each 128KB
 */
...
/*************RTOS Project************/
void LogOut_Flash_Write (uint32_t StartSectorAddress, float * Num)
{

	float2BytesPtr(bytes_temp_buf, Num);

	Flash_Write_Data (StartSectorAddress, (uint32_t *)bytes_temp_buf, 4);
}
void float2BytesPtr(uint8_t * ftoa_bytes_temp,float * float_variable)
{
    union {
      float a[4];
      uint8_t bytes[16];
...
void LogOut_Flash_Read (uint32_t StartSectorAddress,float * Databuf)
{
	uint8_t buffer[16];
	float * value;

	Flash_Read_Data(StartSectorAddress, (uint32_t *)buffer, 4);
	Bytes2floatPtr(buffer,Databuf);

}
void Bytes2floatPtr(uint8_t * ftoa_bytes_temp,float * Databuf)
{
    union {
      float a[4];
      uint8_t bytes[16];
